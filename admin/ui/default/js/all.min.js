...
conf=$.extend({},$.tools.scrollable.conf,conf);this.each(function(){el=new Scrollable($(this),conf);$(this).data("scrollable",el);});return conf.api?el:this;};})(jQuery);var busyImage='<div id="pleasewait"><img src="images/busy.gif" with="34" height="34" border="0" alt="Please wait, processing your request" title="Please wait, processing your request" id="pleasewaitimg" /></div>';var menuArrowImage='ui/dressprow/images/menuarrow.png';var menuArrowActiveImagesrc='ui/dressprow/images/menuarrow_active.png';var loaded=false;var loadCounter=0;var loadMessage='Please wait, your request is being processed. Do not refresh this page. ';var loadMessages=new Array();loadMessages[5]='Still loading';loadMessages[30]='It may seem to take a while, but there is a lot of data to crunch';loadMessages[60]='It should be soon now';loadMessages[90]='This seems to take longer than expected';loadMessages[120]='Still loading, please be patient';loadMessages[150]='It will really be soon now';var currentMessage='';function urlParameter(name,link){name=name.replace(/[\[]/,"\\\[").replace(/[\]]/,"\\\]");var regexS="[\\?&]"+name+"=([^&#]*)";var regex=new RegExp(regexS);var results=regex.exec(link);if(results==null)
return"";else
return results[1];}
...
function getServerTime(){$('#servertime').load('./?page=pageaction&ajaxed=true&action=getservertime',"",function(){});}
...
function loadDivContent(index,url){$("#"+index).html(busyImage+'<div id="loadingprogressbanner"></div>');$("#"+index).load(url);}
...
function loadProgress(){if(loadMessages[loadCounter]){currentMessage=loadMessages[loadCounter]+'<br/>';}
if(loadCounter%50==0){currentMessage+=' * ';}
$("#loadingprogressbanner").html(loadMessage+'<br/><br/>'+currentMessage);loadCounter++;}
...
$(document).ready(function(){$(".note .hide").click(function(){$(this).parents('.note').hide();});$("a.ajaxable").click(function(){var url=this.href;var thispage=urlParameter('page',window.location.href);if(thispage==""){thispage="home";}
...
$("#campaigntitleinput").blur(function(){if(this.value==""){this.value="(no title)";return;}});$("#remoteurlinput").focus(function(){if(this.value=='e.g. https://www.phplist.com/testcampaign.html'){this.value="";}})
$("#remoteurlinput").blur(function(){if(this.value==""){this.value="e.g. https://www.phplist.com/testcampaign.html";return;}
...
$('.actionresult').delay(4000).fadeOut(4000);$('.result').delay(4000).fadeOut(4000);$("#spinner").html(busyImage);$("#stopqueue").click(function(){$("#processqueueoutput").html('Processing cancelled');$("#spinner").html('&nbsp;');$("#stopqueue").hide();$("#resumequeue").show();});$(".updatepluginbutton").click(function(){if(!confirm("Are you sure you want to update this plugin? \nphpList does not currently check on compatibility of the update.\nThis will just fetch the latest version.\nPlease verify before upgrading.")){return false;}
return true;});var docurl=document.location.search;document.cookie="browsetrail="+escape(docurl);setInterval("autoSave();",120000);$('li.selectallcategory').on('click',function(){if($(this).find('input[type=checkbox]').attr('id').match('all-lists')){var ul=$(this).parent();var lists=ul.parent().find('li');checkAllBoxes(lists.find('input[id^=all-lists]').prop('checked'),lists);}});setInterval(getServerTime,30000);if(typeof contentdivcontent!='undefined'&&contentdivcontent!=null&&contentdivcontent!=''){$("#contentdiv").html(busyImage+'<div id="loadingprogressbanner"></div>');$("#contentdiv").load(contentdivcontent);setInterval(loadProgress,1000);}
if(typeof asyncLoad!='undefined'){alert("loading divs");ayncLoad.forEach(loadDivContent)}});function allDone(text){if(!text){text='All done';}
